...
By now you would have all ready installed the required R packages necessary for the successful completion of this module. If you have, please do so now by following the instructions below by pasting them in the R console:
...
The peak on Chr 6 is above the red significance line.
#### Finding LOD peaks
...
One strategy for finding genes related to a phenotype is to search for genes with expression QTL (eQTL) in the same location. Ideally, we would have brain gene expression data in the DO mice from this experiment. Unfortunately, we did not collect this data. However, we have brain (Striatum) gene expression for a separate set of naive DO mice . We searched for genes in the QTL interval that had an eQTL in the same location. Then, we looked at the pattern of founder effects to see if PWK/PhJ stood out in addition to C57BL/6J and CAST/EiJ showing the same pattern. We found one gene that met these criteria.
